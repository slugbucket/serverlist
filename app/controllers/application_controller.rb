class ApplicationController < ActionController::Base
  ...
  rescue_from ActiveRecord::RecordInvalid, with: :db_invalid_stmt
  ...
  def record_not_unique(exception)
    session.delete :search
    redirect_to root_url, :alert => "Requested record is not unique: #{exception.message}."
  end
  ...
  # Provide a replacement for the default user method expected by CanCan
   #alias_method :current_user, :current_session
   # or,
   def current_ability
     @current_ability ||= Ability.new(current_session)
   end
end
